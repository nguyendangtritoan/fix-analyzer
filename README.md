...
* **Dynamic Dictionary:** Ships with a standard FIX 4.4 subset but supports runtime injection of custom QuickFIX XML dictionaries (`FIX44.xml`, `FIX50.xml`) to override tags and enums.
...
* **Nested Repeating Groups:** The current diff logic flattens repeating groups. It does not visually nest sub-groups (e.g., a `NoPartyIDs` inside a `NoSides`).
* **Performance:** Extremely large messages (1MB+) might cause React render lag due to the large DOM table size. Virtualization could be added to `DiffView` if needed.
